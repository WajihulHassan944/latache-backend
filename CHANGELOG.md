...
- **Emails:**
  - The "Your security is our priority / Never share this code or your password" panel and the shield artwork now appear only on the verification and password-reset OTP emails. The admin-welcome email keeps its own credential warning. Booking, payment, plan and dispute emails no longer show them.
  - Notification emails show a `Booking #id` reference only when the email really is about a booking. Before, custom-time, plan and settlement emails labelled an internal id as a booking.
  - Dispute emails show the short `DSP-XXXXXX` reference, a readable event label and an "open the app" line.
  - OTP plain-text versions include the never-share line.
...
- **`awaiting_payment` bookings now expire.** When a Tasker accepts an online-paid booking, `Booking.paymentDueAt` is set to now + `bookingRules.awaitingPaymentExpiryMinutes` (default 60, env `BOOKING_AWAITING_PAYMENT_EXPIRY_MINUTES`). It is capped at the booking start and is never less than 5 minutes. A new `bookings.expire-awaiting-payment` sweep (every minute) cancels unpaid bookings past the deadline (`cancelledByRole: 'system'`), releases the slot and any referral discount reservation, and notifies both sides (`booking_payment_expired`). `paymentDueAt` is exposed on customer booking and Tasker task views while `status` is `awaiting_payment`. The customer's "Payment required" notification states the deadline. Bookings already awaiting payment at deploy time got a fresh 60-minute window.
- `POST /api/bookings/:id/complete-payment` refuses new payment attempts after `paymentDueAt` (409 `BOOKING_PAYMENT_WINDOW_EXPIRED`); an already-succeeded Stripe attempt is still honored.
- Fixed: the Stripe webhook ignored acceptance-capture PaymentIntents, so a 3DS payment completed outside the app left the booking in `awaiting_payment`. `payment_intent.succeeded` for these intents now confirms the booking. Before expiring a card booking, the sweep checks Stripe: it confirms a payment that actually succeeded, waits on one that is still processing, and cancels an abandoned intent. A payment that lands after the booking was already cancelled is refunded automatically (`booking_payment_refunded`).
- Paid plans: new `POST /api/tasker/plans/cancel` (a pending purchase is cancelled and refunded; an active plan stops renewing and ends at `nextBillingAt` without another charge) and `POST /api/tasker/plans/resume`. `GET /api/tasker/plans/active` additionally returns `nextBillingAt`, `autoRenew`, `graceUntil`. New plan status `cancelled`.
- Custom-time requests: new `GET /api/custom-time-requests` (Tasker inbox / Customer outbox, `status` filter, paginated) and `POST /api/custom-time-requests/:id/cancel` (Customer withdraws a pending/accepted request, freeing the one-pending-per-Tasker limit; Tasker is notified). New request status `cancelled`.

# 3.39.0

- **Custom-time booking requests.** New `POST /api/taskers/:taskerId/custom-time-request` (Customer), `POST /api/custom-time-requests/:requestId/respond` (Tasker, `{ accept }`), and `GET /api/custom-time-requests/:requestId` (either participant). A Customer asks for a date/time outside the Tasker's listed availability; the Tasker has `customTimeRequests.ttlMinutes` (default 30 minutes) to accept/reject, after which a new `bookings.expire-custom-time-requests` sweep expires it. Acceptance resets `expiresAt` to a fresh 30-minute booking window. One pending request per Customer/Tasker pair (409, backed by a partial unique index).
- `POST /api/bookings/quote` and `POST /api/bookings` accept an optional `customTimeRequestId`. When valid (owned by the caller, accepted, unexpired, unfulfilled, and matching taskerId/serviceSlug/date/time) the open-slot match is skipped; `POST /api/bookings` re-validates under a row lock and marks the request `fulfilled`. The booking is carried on a dedicated `UserAvailabilities` row flagged `isCustom` (never shown as open availability, excluded from discovery/quote/reschedule), lasting the minimum billable duration. Quote stays public; a bearer token is only required when `customTimeRequestId` is sent.
- Quote/booking now also reject a listed slot that overlaps a Tasker's existing active booking (needed once custom-time bookings can sit alongside listed slots).
- **Paid Tasker plans (Gold/Platinum/Diamond).** New `GET /api/tasker/plans` (catalog), `GET /api/tasker/plans/active`, `POST /api/tasker/plans/:planId/purchase` (`paymentMethod`: `wallet` | `stripe` | saved PaymentMethod id; 402 on insufficient balance/decline, no fallback). Purchases always land `pending`; admins review via `GET /api/admin/tasker-plans`, `POST /api/admin/tasker-plans/:id/approve|reject` (`elite.read`/`elite.manage`). Rejection refunds to the original method. Separate `TaskerSubscriptions` table, unrelated to the earned Elite tier.
- Active plan perks are applied automatically: platform fee override (12/10/8%, used only when lower than the Tasker's existing rate) in `calculatePricingCharges`; discovery ordering weight (`planSearchRank`, after Elite search rank); `planBadge` on `GET /api/taskers` and `GET /api/taskers/:id`; monthly bonus (25/60/150 MAD) credited to the Tasker wallet at activation and each renewal.
- Plans auto-renew every 30 days on the purchase method via a new `tasker-plans.maintain` sweep. A failed renewal keeps perks during a 3-day grace period (`taskerPlans.graceDays`) with daily retries, then the plan becomes `expired` and perks stop.
- Tasker availability (`POST /api/taskers/onboarding`, `POST /api/tasker-dashboard/profile/availability`) now accepts **today** as well as future dates. A slot dated today whose start time is at or before the current UTC time is rejected; past dates and all overlap/booked-slot rules are unchanged.
