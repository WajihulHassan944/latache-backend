...
export class EliteProgramService {
  ...
  async overview(query: AdminDateRangeQueryDto): Promise<Record<string, unknown>> {
    ...
    return {
      history: {
        ...
        note:
          'Taskers marked isElite before tiered membership existed were migrated to Gold without fabricated historical transitions.',
      },
      metrics: {
        totalElite,
        tiers: tierCounts,
  ...
  async taskerState(taskerId: number): Promise<Record<string, unknown>> {
    const tasker = await this.prisma.user.findFirst({
      where: { id: taskerId, role: UserRole.Tasker, deletedAt: null },
      include: {
        eliteTier: { include: { benefits: { where: { isActive: true }, orderBy: { sortOrder: 'asc' } } } },
    ...
    if (!tasker) throw new NotFoundException('Tasker not found');
    const tiers = await this.prisma.eliteTier.findMany({ where: { isActive: true }, orderBy: { rank: 'asc' } });
    const currentIndex = tasker.eliteTier ? tiers.findIndex((tier) => tier.id === tasker.eliteTierId) : -1;
    ...
    return {
      ...
      pendingRequest: tasker.eliteRequests[0] ? this.serializeRequest(tasker.eliteRequests[0]) : null,
      availableActions: {
        ...
        nextTier: nextTier ? this.serializeTier(nextTier) : null,
        previousTier: previousTier ? this.serializeTier(previousTier) : tasker.isElite ? { code: 'standard', name: 'Standard' } : null,
      },
      eligibility: {
        targetTier: eligibilityTarget ? this.serializeTier(eligibilityTarget) : null,
        ...eligibility,
        note:
  ...
  async submitRequest(taskerId: number, dto: TaskerEliteRequestDto): Promise<Record<string, unknown>> {
    ...
    try {
      request = await this.prisma.$transaction(async (tx) => {
        ...
        if (!tasker.isVerified || !tasker.isDocVerified || tasker.onboardingStatus !== 'approved') {
          throw new ConflictException('Tasker onboarding and identity verification must be approved first');
        }

        const existing = await tx.eliteMembershipRequest.findFirst({
          where: { taskerId, status: 'pending' },
        });
        if (existing) throw new ConflictException('A pending Elite Program request already exists');
  ...
  async decide(actor: User, requestId: string, dto: EliteDecisionDto): Promise<Record<string, unknown>> {
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (request.status !== 'pending') throw new ConflictException('Elite request is no longer pending');

      const currentTier = request.tasker.eliteTier?.code ?? null;
      if (currentTier !== request.fromTierCode) {
        throw new ConflictException('Tasker tier changed after this request was submitted; review a fresh request');
      }
      ...
      if (dto.action === 'reject') {
        ...
        await this.audit.record({
          ...
          entityId: request.id,
          reason: dto.reason,
          metadata: { kind: request.kind, fromTier: request.fromTierCode, toTier: request.toTierCode },
        }, tx);
  ...
  async setTier(actor: User, taskerId: number, dto: SetEliteTierDto): Promise<Record<string, unknown>> {
  ...
  async program(): Promise<Record<string, unknown>> {
    const [tiers, badges] = await Promise.all([
      this.prisma.eliteTier.findMany({
        orderBy: { rank: 'asc' },
        include: {
          benefits: { orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }] },
    ...
    return {
      ...
      benefitEnforcement: {
        ...
        note:
          'Benefit definitions are real persisted program configuration. Financial/booking effects are applied only when a consuming module explicitly integrates the benefit code; this API never fabricates discounts or bonuses.',
      },
    };
  }
  ...
  async replaceBenefits(
    ...
    const duplicates = normalized.filter(
      (benefit, index) => normalized.findIndex((candidate) => candidate.code === benefit.code) !== index,
    );
    if (duplicates.length) throw new BadRequestException('Benefit codes must be unique within a tier');

    const tier = await this.prisma.eliteTier.findUnique({ where: { code: this.normalizeTierCode(tierCode) } });
    ...
    return { tier: this.serializeTier(tier), benefits: benefits.map((benefit) => this.serializeBenefit(benefit)) };
  }
  ...
  async createBadge(actor: User, dto: CreateEliteBadgeDto): Promise<Record<string, unknown>> {
    ...
    try {
      const badge = await this.prisma.eliteBadge.create({
        data: {
          tierId: tier?.id ?? null,
          code,
          name: dto.name.trim(),
      ...
      await this.audit.record({
        ...
        entityId: badge.id,
        metadata: { code: badge.code, tier: badge.tier?.code ?? null },
      });
  ...
  async updateBadge(actor: User, badgeId: string, dto: UpdateEliteBadgeDto): Promise<Record<string, unknown>> {
    ...
    const tier = dto.tier
      ...
      : undefined;
    if (dto.tier && !tier) throw new NotFoundException('Elite tier not found');
    const badge = await this.prisma.eliteBadge.update({
      where: { id: badgeId },
      data: {
  ...
  async assignBadge(actor: User, taskerId: number, badgeId: string): Promise<Record<string, unknown>> {
    ...
    if (!tasker.isElite || !tasker.eliteTier) throw new ConflictException('Only current Elite taskers can receive Elite badges');
  ...
  async revokeBadge(
    ...
    if (!assignment || assignment.revokedAt) throw new NotFoundException('Active tasker badge assignment not found');
  ...
  async performance(query: AdminDateRangeQueryDto): Promise<Record<string, unknown>> {
    ...
    const elite = await this.prisma.user.findMany({
      ...
      select: { id: true, firstName: true, lastName: true, profilePicture: true, rating: true, completedTasks: true, eliteTier: true },
    });
    const ids = elite.map((tasker) => tasker.id);
    if (!ids.length) {
      return {
        range: this.rangeView(range),
        metrics: { totalElite: 0, averageRating: 0, completionRate: 0, settledEarnings: 0, platformFees: 0, disputes: 0 },
        trend: [],
        topPerformers: [],
      };
    }
    ...
    const [statuses, earnings, fees, disputes, earningRows, bookingRows] = await Promise.all([
      ...
      this.prisma.taskerWalletLedgerEntry.aggregate({
        where: { taskerId: { in: ids }, kind: 'earning', status: 'settled', ...(date ? { createdAt: date } : {}) },
        _sum: { amount: true },
      }),
      ...
      this.prisma.booking.findMany({
        where: { taskerId: { in: ids }, status: { in: ['completed', 'cancelled'] }, ...(date ? { createdAt: date } : {}) },
    ...
    const averageRating = Number((elite.reduce((sum, tasker) => sum + Number(tasker.rating), 0) / elite.length).toFixed(2));

    const earningsByTasker = new Map<number, number>();
    for (const row of earningRows) {
      earningsByTasker.set(row.taskerId, (earningsByTasker.get(row.taskerId) ?? 0) + Number(row.amount));
    }
  ...
  async reportData(
    ...
    if (type === 'monthly_summary') {
      const [overview, performance] = await Promise.all([this.overview(query), this.performance(query)]);
    ...
    const tiers = await this.prisma.eliteTier.findMany({
      orderBy: { rank: 'asc' },
      include: { benefits: { where: { isActive: true }, orderBy: { sortOrder: 'asc' } }, _count: { select: { members: true } } },
    });
    return {
      type,
      range: this.rangeView(range),
      ...
      note:
        'Benefit usage is not fabricated. Utilization remains unavailable until a consuming booking/payment/support feature records real benefit usage events.',
  ...
  private async listMembers(query: ListEliteAdminDto): Promise<Record<string, unknown>> {
    ...
    let items = members.map((member) => ({
      ...
      badges: member.eliteBadges.map((item) => ({ id: item.badge.id, code: item.badge.code, name: item.badge.name, assetUrl: item.badge.assetUrl })),
    }));
    if (query.sort === 'earnings_desc') {
      items = items.sort((a, b) => b.settledEarnings - a.settledEarnings);
    }
    return { view: 'members', page, limit, totalItems, totalPages: Math.ceil(totalItems / limit), items };
  }
      ...
      items: requests.map((request) => {
        ...
        return {
          ...
          decidedBy: request.decidedBy
            ? { id: String(request.decidedBy.id), name: fullName(request.decidedBy.firstName, request.decidedBy.lastName) }
            : null,
        };
      }),
    };
  ...
  private async resolveRequestedTarget(
    ...
    const tiers = await tx.eliteTier.findMany({ where: { isActive: true }, orderBy: { rank: 'asc' } });
    if (!tiers.length) throw new ConflictException('No active Elite tiers are configured');
    if (kind === 'application') {
      if (currentTier) throw new ConflictException('Current Elite members cannot submit a new application');
      return tiers[0] ?? null;
    }
    if (!currentTier) throw new ConflictException(`${kind} requests require current Elite membership`);
  ...
  private async applyTierChange(
    tx: Prisma.TransactionClient,
    input: {
      tasker: { id: number; isElite: boolean; eliteSince: Date | null; eliteTier: { code: string } | null };
  ...
  private async metricsSnapshot(client: DbClient, taskerId: number): Promise<MetricsSnapshot> {
    const [tasker, statuses, earnings, openComplaints] = await Promise.all([
      client.user.findUnique({ where: { id: taskerId }, select: { rating: true, completedTasks: true } }),
  ...
  private reconstructTierHistory(
    ...
    for (const key of keys) {
      ...
      for (const transition of buckets.get(key) ?? []) {
        if (transition.toTierCode) counts.set(transition.toTierCode, Math.max(0, (counts.get(transition.toTierCode) ?? 0) - 1));
        if (transition.fromTierCode) counts.set(transition.fromTierCode, (counts.get(transition.fromTierCode) ?? 0) + 1);
      }
    }
  ...
  private performanceTrend(
    ...
    const bucket = (date: Date) => granularity === 'day' ? date.toISOString().slice(0, 10) : date.toISOString().slice(0, 7);
  ...
  private serializeTier(tier: { id: string; code: string; name: string; rank: number; description: string | null; requirements?: unknown; isActive?: boolean }) {
    return {
      id: tier.id,
      code: tier.code,
      name: tier.name,
  ...
  private serializeBenefit(benefit: { id: string; code: string; name: string; description: string | null; displayValue: string | null; metadata: unknown; isActive: boolean; sortOrder: number }) {
    return {
      id: benefit.id,
      code: benefit.code,
      name: benefit.name,
  ...
  private serializeBadge(badge: { id: string; code: string; name: string; description: string | null; assetUrl: string | null; criteria: unknown; isActive: boolean; tier?: { code: string; name: string } | null }) {
    return {
      id: badge.id,
      code: badge.code,
      name: badge.name,
  ...
  private serializeRequest(request: { id: string; kind: string; fromTierCode: string | null; toTierCode: string | null; status: string; reason: string | null; decisionReason: string | null; metricsSnapshot: unknown; requirementsSnapshot: unknown; createdAt: Date; updatedAt: Date; decidedAt: Date | null; cancelledAt: Date | null }) {
    return {
      id: request.id,
      kind: request.kind,
      fromTier: request.fromTierCode,
  ...
  private evaluateEligibility(
    ...
    if (requirements.maxOpenComplaints !== undefined) {
      ...
      const score = passed
        ...
        : required <= 0
          ? 0
          : Math.min(100, (required / actual) * 100);
      checks.push({
  ...
  private asMetricsSnapshot(value: unknown): MetricsSnapshot | null {
    ...
    return {
      ...
      settledEarnings: values.settledEarnings!,
      openComplaints: values.openComplaints!,
  ...
  private rangeView(range: { from: Date | null; toExclusive: Date | null; range: string; granularity: string }) {
    return {
      range: range.range,
